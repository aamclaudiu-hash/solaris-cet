...
| File | Purpose |
|------|---------|
| [check-dist-for-e2e.mjs](./check-dist-for-e2e.mjs) | `pretest:e2e` from `app/` — fails fast if `app/dist/index.html` is missing (run `npm run build` before `npm run test:e2e`) |
