...
test.describe('AI Model Execution — Intelligence Core', () => {
  ...
  test('ReAct cycling advances to a new active step', async ({ page }) => {
    ...
    // The active step has opacity:1 and a box-shadow glow on its phase label span.
    // We read the active step index from the first fully-opaque step row.
    const getActiveText = async () => {
      return page.evaluate(() => {
        const rows = document.querySelectorAll<HTMLElement>('[class*="transition-all duration-500"]');
    ...
    // Wait 2.5 s (slightly longer than the 2 s interval) and check for a change
    await page.waitForTimeout(2500);
    const second = await getActiveText();
    // Either the step advanced or wrapped — either way we confirm the cycle is running
