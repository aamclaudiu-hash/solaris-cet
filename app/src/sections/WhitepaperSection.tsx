...
const wpSections: WPSection[] = [
  ...
  },
  {
    ...
    content: [
      'The RAV (Reason-Act-Verify) Protocol is the proprietary operational framework powering every Solaris CET agent. Before any on-chain action is executed, the agent must produce a verifiable reasoning trace stored immutably on IPFS and anchored to its Layer 1 transaction. This military-grade precision model ensures every decision is auditable, reproducible, and disputable.',
      'The loop has three phases: REASON (decompose the goal into sub-objectives using BRAID graphs and index all available on-chain and off-chain signals), ACT (execute the optimal action sequence across market automation or crop diagnostics pipelines and record all parameters on-chain), VERIFY (confirm the outcome matches the prediction, update the agent\'s self-model, and publish a verifiable performance delta).',
