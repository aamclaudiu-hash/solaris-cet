...
The technical infrastructure underpinning Solaris CET is purpose-built for the TON blockchain's unique asynchronous, actor-based execution environment. The Jetton Master Contract (`EQBbUfeIo6yrNRButZGdf4WRJZZ3IDkN8kHJbsKlu3xxypWX`), implemented in strict conformance with the TEP-74 Fungible Token Standard, governs all on-chain token operations. Treasury custody is entrusted exclusively to a Wallet V5 R1 multi-signature address (`UQDUP2y5HBR1tRA3cZ6spDl2PV-KE2Wts_To5JTQQEf2favu`), which enforces multi-party authorization for every outbound treasury transaction—a structural safeguard against the unilateral-key drain scenarios that have historically hit DAO treasuries across Ethereum, Solana, and BNB Chain ecosystems.
...
### 2.2 Legacy Treasury Custody Gaps — And the Multi-Sig Countermeasure
...
The Master Contract (`EQBbUfeIo6yrNRButZGdf4WRJZZ3IDkN8kHJbsKlu3xxypWX`) exposes the following standardized operations:
...
Solaris combines TON's actor-model isolation, audited Jetton contracts, and multi-signature treasury controls to maximize defensive depth. Nevertheless, all smart contracts carry residual technical risk: bugs, unexpected interactions, or edge-case failure modes in the Jetton Master Contract, Jetton Wallet contracts, the V5 R1 multi-sig implementation, or the DeDust.io AMM contracts could potentially result in partial or total loss of funds. Users interact with all contracts entirely at their own risk. The Solaris project commits to ongoing security review and transparent public disclosure of any identified security issues, but cannot guarantee the complete absence of undiscovered flaws. No insurance or indemnification is provided for smart contract losses.
...
|------------------------------|------------------------------------------------------------------------------------|
| Jetton Master Contract       | `EQBbUfeIo6yrNRButZGdf4WRJZZ3IDkN8kHJbsKlu3xxypWX`                                     |
